...
Libgit2 **v1.2.x**; binary wheels already include libgit2, so you only need to
...
.. code-block:: sh
   ...
   $ wget https://github.com/libgit2/libgit2/archive/refs/tags/v1.2.0.tar.gz -O libgit2-1.2.0.tar.gz
   $ tar xzf libgit2-1.2.0.tar.gz
   $ cd libgit2-1.2.0/
   $ cmake .
   $ make
   $ sudo make install
...
.. code-block:: sh
   ...
   $ git clone --depth=1 -b maint/v1.2 https://github.com/libgit2/libgit2.git
   $ cd libgit2
