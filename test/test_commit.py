...
def test_read_commit(barerepo):
    ...
    assert parents[0].id == 'c2792cfa289ae6321ecf2cd5806c2194b0fd070c'
    ...
    assert commit.tree.id == '967fce8df97cc71722d3c2a5930ef3e6f1d27b12'
...
def test_new_commit(barerepo):
    ...
    assert commit.id == '98286caaab3f1fde5bf52c8369b2b0423bad743b'
    assert commit.message_encoding is None
    assert message == commit.message
    assert commit.commit_time == 12346
    ...
    assert commit.parents[0].id == COMMIT_SHA
...
def test_new_commit_encoding(barerepo):
    ...
    assert commit.type == ObjectType.COMMIT
    assert commit.message_encoding == 'iso-8859-1'
...
def test_amend_commit_metadata(barerepo):
    ...
    assert amended_commit.type == ObjectType.COMMIT
