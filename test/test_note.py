...
class NotesTest(utils.BareRepoTestCase):
    ...
    def test_create_note(self):
        ...
        note = self.repo.create_note("note message", author, committer, annotated_id)
        self.assertEqual("note message", note.git_note_message)

    def test_lookup_note(self):
        annotated_id = self.repo.head.hex
        note = self.repo.lookup_note(annotated_id)
        self.assertEqual(NOTES[0][0], note.git_note_oid)
        self.assertEqual(NOTES[0][1], note.git_note_message)
    ...
    def test_iterate_notes(self):
        for i, note in enumerate(self.repo.notes()):
            entry = (note.git_note_oid, note.git_note_message, note.annotated_id)
